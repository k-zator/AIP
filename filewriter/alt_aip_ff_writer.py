...
from filewriter.compute_virtual_sites import get_average_weights, get_average_anchors
...
class AipFFWriter(AipReader):
    ...
    def get_vs_tree(self):
        ...
        for i, ssip in enumerate(self.list_aips):
            ...
            vs_element.attrib["weight1"] = str(round(ssip.weights[0], 3))
            vs_element.attrib["weight2"] = str(round(ssip.weights[1], 3))
            vs_element.attrib["weight3"] = str(round(ssip.weights[2], 3))
        return vs_tree
    ...
    def get_xml_tree(self):
        ...
        for k, v in self.dict_atoms.items():
            ...
            atom_elem.attrib["atom_neigh"] = str(k[1:])
    ...
    def get_custom_xml(self, ligand_tree, protein_xml=None):
        ...
        perpar_elem = etree.SubElement(custom_elem, "PerParticleParameter")
        perpar_elem.attrib["name"] = "atom_neigh"
        ...
        attfrom_elem = etree.SubElement(custom_elem, "UseAttributeFromResidue")
        attfrom_elem.attrib["name"] = "atom_neigh"
    ...
    def set_anchors_weights(self, list_aips):
        ...
        for aip in list_aips:
            anchor1, anchor2, anchor3 = get_average_anchors(self.dict_atoms)
            weights = get_average_weights(aip, anchor1, anchor2, anchor3)
