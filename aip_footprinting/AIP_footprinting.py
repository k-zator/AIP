...
class KMC_footprint():
    ...
    def __init__(self, mol, centre_surface_percentile=90, lp_excl_r=1.5, dualAIP=False,
        ...
        try:  # parsing the name using the cross-check dictionary (ccd)
            inchikey = ccd[mol]
        except Exception:
            LOGGER.error("\n The molecule name input is not valid")
            sys.exit()
